...
public class ProductService {
		...
		if(product.isPresent()) {
			...
			ProductMapper mapper=new ProductMapper();
			 productDTO=mapper.convertProductToProductDTO(product.get());
				/*
				 * logger.info("calling tax service to get the tax for given product name:"
				 * +productDTO.getProductName()); Long tax=
				 * restClient.getTax(productDTO.getProductName());
	 ...
	 * 
	 * Optional<Product> product= productRepository.findById(id); ProductDTO
	 * productDTO=null; if(product.isPresent()) {
	 * 
	 * ProductMapper mapper=new ProductMapper();
	 * productDTO=mapper.convertProductToProductDTO(product.get());
